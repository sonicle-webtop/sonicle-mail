...
public class SaxHTMLMailParser extends DefaultHandler implements LexicalHandler {
  ...
  private SaxHTMLMailEvaluator evaluator;
  ...
  public void initialize(SaxHTMLMailEvaluator evaluator, boolean justBody, boolean removeHeadStyle) throws SAXException {
    release();
  ...
  public void release() {
    try {
      ...
      if(reader!=null) {
		...
		writer=null;
		reader=null;
		throw new MimeMessageParseException("Error releasing SaxHTMLMailParser", exc);
    }
    writer=null;
    reader=null;
  }
  ...
  public void endOfFile() {
	try {
		...
		throw new MimeMessageParseException("Error in SaxHTMLMailParser at end of file", exc);
	}
  }
  ...
  private void evaluateCid(String avalue) {
	  if (evaluator != null) {
		String name=avalue.substring(4);
		evaluator.evaluateCid(name);
	  }
  }
  ...
  private String evaluateUrl(String avalue) {
	  if (evaluator != null) {
		return evaluator.evaluateUrl(avalue, baseUrl);
	  }
	  return avalue;
  }
