 ...
 * @author gbulfon
 */
public class SieveScriptGenerator implements MailRulesParserListener {
	...
	public StringBuffer generate(MailRules filters, String spamFolder) {
		rows=0;
		this.spamFolder=spamFolder;
		MailRulesParser.parse(filters, this);
		return sievescript;
	}
	...
	public void rule(int row, MailRuleConditions mfcs) {
		...
		if (mfcs.getOperator().equals(MailRuleConditions.OPERATOR_OR)) sievescript.append("anyof");
		...
		for(int i=0;i<mfcssize;++i) {
			...
			MailRuleCondition mfc=mfcs.get(i);
	...
	private boolean isSpamRule(MailRuleConditions mfcs) {
		...
		if (mfcs.size()==1 && mfcs.action.equals("FILE") && mfcs.actionvalue!=null && spamFolder!=null && mfcs.actionvalue.equals(spamFolder)) {
			MailRuleCondition mfc=mfcs.get(0);
